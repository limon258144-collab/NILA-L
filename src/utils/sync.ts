export async function syncWithServer(updatePayload?: any) {
  try {
    // 1. Gather all current client-side local storage states so they are never lost
    let regUsers = {};
    try {
    ...
    // 2. Build combined payload, with any explicit updates overriding
    const fullPayload = {
    ...
    // 3. Perform POST sync with server
    ...
    if (res.ok) {
      ...
      // Dispatch event to render updates across screen mounts instantly
